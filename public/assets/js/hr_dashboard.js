...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const data = window.HR_CHART_DATA || {};
  const COLORS = {
    green: "#0F5E3D",
    amber: "#f59e0b",
    red: "#ef4444",
  ...
  if (statusCanvas && data.status) {
    const { pending, approved, rejected, cancelled } = data.status;
  ...
  const trendCanvas = document.getElementById("hrTrendChart");
  if (trendCanvas && data.trend) {
    const ctx = trendCanvas.getContext("2d");
    const gradient = ctx.createLinearGradient(0, 0, 0, 160);
    gradient.addColorStop(0, "rgba(15, 94, 61, 0.22)");
  ...
  const typesCanvas = document.getElementById("leaveTypesChart");
  if (typesCanvas && data.types) {
